...
import { convertHtmlToPdf } from "../../services/integrationApi.js";
...
export default function HtmlToPdfPage() {
  ...
  const onPickFile = (f) => {
    if (!f) return;
    const okMime = (f.type || "").toLowerCase().startsWith("text/html");
    const okExt = /\.html?$/i.test(f.name || "");
    if (!okMime && !okExt) {
      setToast({ message: "Choose an HTML file (.html / .htm).", hidden: false });
      return;
    }
    setFile(f);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (!file) {
      setToast({ message: "Choose an HTML file first.", hidden: false });
      return;
    }
    setBusy(true);
    setToast({ message: "Converting to PDF…", hidden: false });
    try {
      const { blob, filename } = await convertHtmlToPdf(file, {
  ...
  return (
    <>
      <section className="dash__panel" aria-labelledby="htmlpdf-title">
        <div className="dash__panel-head">
          <div className="dash__pill">ApyHub</div>
          <h2 id="htmlpdf-title" className="dash__panel-title">
            HTML → PDF
          </h2>
          <p className="dash__panel-subtitle">
            Upload an HTML file and download a PDF while preserving formatting. Your provider token stays server-side.
          </p>
        </div>
        ...
        <form className="integration__form" onSubmit={onSubmit}>
          ...
          <div className="integration__row">
            <div className="field">
              <label className="label" htmlFor="html-out-name">
                Output filename
              </label>
      ...
      <aside className="dash__side">
        <div className="side__card">
          <h3 className="side__title">Tips</h3>
          <ul className="side__list">
            <li>Keep external assets (images/fonts) embedded or publicly reachable.</li>
            <li>If your HTML is large, conversion can take longer.</li>
          </ul>
        </div>
      </aside>
