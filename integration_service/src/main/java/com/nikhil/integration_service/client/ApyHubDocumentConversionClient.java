...
@Component
@AllArgsConstructor
public class ApyHubDocumentConversionClient implements DocumentConversionClient {
    ...
    public byte[] convertImageToPdf(
        ...
        if (!ImageFormats.isAllowedForApyHub(originalFilename, contentType)) {
            throw new ThirdPartyIntegrationException(PROVIDER, "invalid_input",
                    "ApyHub accepts JPEG, JPG, and PNG only (filename extension or image/jpeg | image/png).");
        }

        String out = (outputFilename == null || outputFilename.isBlank()) ? "output.pdf" : outputFilename.trim();
        String url = props.baseUrl()
                + "/convert/image-file/pdf-file"
                + "?output=" + URLEncoder.encode(out, StandardCharsets.UTF_8)
                + "&landscape=" + landscape;

        MediaType filePartType = ImageFormats.resolvePartMediaType(originalFilename, contentType);
        MultipartBodyBuilder builder = new MultipartBodyBuilder();
        builder.part("file", new NamedByteArrayResource(imageBytes, originalFilename))
                .contentType(filePartType);
